...
const defaults = {
  ...
  theme_color: 'default',
  auto_bookmark: true,
  auto_unlock_next_chapter: false,
};
...
async function upsert(userId, prefs) {
  ...
  const values = {
    ...
    auto_unlock_next_chapter: prefs.auto_unlock_next_chapter
      ?? existing?.auto_unlock_next_chapter
      ?? defaults.auto_unlock_next_chapter,
  };

  const result = await db.query(
    `INSERT INTO user_preferences (
       user_id, dark_mode, font_size, line_spacing, font_family, theme_color,
       auto_bookmark, auto_unlock_next_chapter
     )
     ...
     DO UPDATE SET
       ...
       auto_unlock_next_chapter = EXCLUDED.auto_unlock_next_chapter,
    ...
    [
      ...
      values.auto_unlock_next_chapter,
    ]
  );
  return result.rows[0];
}

module.exports = { getByUserId, upsert, defaults };

...
ERD do nhóm cung cấp có đồng thời \CodePath{ratings} và \CodePath{story_ratings}, trong khi schema chính được rà soát sử dụng \CodePath{ratings}. Trước khi nộp, nhóm cần xác nhận bảng đang được ứng dụng sử dụng, loại bỏ bảng dư hoặc giải thích rõ mục đích của từng bảng; đồng thời bảo đảm ERD, migration và cơ sở dữ liệu triển khai cùng một phiên bản.
...
\subsection{Nợ kỹ thuật dữ liệu}

Phần dữ liệu hiện có ba cơ chế cùng tồn tại: schema tổng, script bổ sung và một số DDL chạy khi module database kết nối. Cách tổ chức này làm nguồn sự thật bị phân tán và khó tái tạo một cơ sở dữ liệu mới. Bảng \CodePath{reports} dùng script \CodePath{CREATE TYPE} và \CodePath{CREATE TABLE} chưa có \CodePath{IF NOT EXISTS}; \CodePath{bad_words} xuất hiện trên ERD nhưng chưa có DDL trong phần mã nguồn đã rà soát. Hướng hoàn thiện là:

\begin{itemize}
  \item đưa mọi thay đổi vào migration đánh số và có thể chạy lặp an toàn;
  \item tạo migration chính thức cho \CodePath{bad_words}, \CodePath{reports}, \CodePath{tags}/\CodePath{story_tags};
  \item xác nhận và xử lý sự trùng lặp giữa \CodePath{ratings} và \CodePath{story_ratings};
  \item loại DDL biến đổi schema khỏi đường khởi động ứng dụng sau khi migration ổn định;
  \item bổ sung bảng theo dõi migration và kiểm thử khởi tạo DB trắng;
  \item ghi rõ baseline schema tương ứng với commit dùng để nộp.
\end{itemize}
...
\subsection{Thứ tự ưu tiên hoàn thiện}

Nhóm cần hoàn tất metadata, phân công, URL và ảnh thật; sau đó chuẩn hóa migration, xử lý Redis/open handles, chạy E2E và tối ưu bundle.

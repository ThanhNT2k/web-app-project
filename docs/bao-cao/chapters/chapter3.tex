...
Giao diện đọc chương và các tùy chọn đọc tập trung hiển thị nội dung rõ ràng, đồng thời cung cấp điều hướng giữa các chương và tùy chỉnh như cỡ chữ, kiểu chữ hoặc chế độ hiển thị để nâng cao trải nghiệm đọc.Khi người dùng truy cập chương trả phí chưa mở khóa, trình đọc chỉ hiển thị phần xem trước cùng thẻ thông báo khóa. Thẻ này cho biết chi phí mở khóa, số dư Tinh thạch hiện có và nút mở khóa chương. Danh sách chương tại trang chi tiết cũng phân biệt rõ chương miễn phí, chương đã mở và chương đang bị khóa để người dùng chủ động lựa chọn.
\ReportImage[1.0\textheight]
...
\ReportImage[1.0\textheight]
  ...
  {Chụp bằng Moderator/\allowbreak{}Admin demo; không thao tác trên dữ liệu cần bảo toàn khi chỉ chụp minh chứng.}
...
\CodePath{Services/api.js} tạo một Axios instance có base URL từ biến môi trường, fallback về local khi phát triển. Request interceptor đọc \CodePath{cmc_token} và gắn Bearer token; response interceptor xử lý 401/\allowbreak{}403, xóa phiên và điều hướng về login trong các trường hợp phù hợp. Các endpoint được gom theo domain như auth, stories, chapters, comments, follows, reports, admin và moderator.
...
Tùy chọn \CodePath{auto_unlock_next_chapter} hiện được lưu vào thiết lập người dùng. Tuy nhiên, phiên bản hiện tại chưa tự động gọi API mở khóa khi chuyển sang chương tiếp theo; đây là hạng mục cần hoàn thiện trước khi công bố là chức năng tự động.
...
\hline
25 & Mở khóa chương trả phí & Chapter khóa; số dư đủ & HTTP \texttt{200},\newline\texttt{CHAPTER\_ALLOWBREAK{}UNLOCKED} & Trừ đúng \CodePath{UNLOCK\_COST} và trả số dư mới & Chương được mở, số dư giảm đúng chi phí cấu hình (\textbf{Đạt}) \\
\hline
...
\hline
27 & Số dư không đủ khi mở khóa & Balance nhỏ hơn \CodePath{UNLOCK\_COST} & HTTP \texttt{409},\newline\texttt{INSUFFICIENT\_CRYSTALS} & Không mở khóa; trả số dư hiện tại & API trả \texttt{409}, số dư không thay đổi (\textbf{Đạt}) \\
\hline

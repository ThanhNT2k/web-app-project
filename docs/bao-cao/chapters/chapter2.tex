...
\centering
\begin{tikzpicture}[
  actor/.style={draw,rounded corners=3pt,minimum width=2.4cm,minimum height=0.8cm,align=center,font=\small\bfseries,fill=white},
  ...
  internal/.style={draw,fill=white,rounded corners=2pt,minimum width=2.5cm,minimum height=0.65cm,align=center,font=\scriptsize,inner sep=2pt}
]
\node[actor] (u) at (0,0) {Người dùng};
\node[actor] (fe) at (3.8,0) {React};
\node[actor] (api) at (7.6,0) {Express API};
\node[actor] (db) at (11.4,0) {PostgreSQL};

\draw[lifeline] (u.south) -- (0,-11.1);
\draw[lifeline] (fe.south) -- (3.8,-11.1);
\draw[lifeline] (api.south) -- (7.6,-11.1);
\draw[lifeline] (db.south) -- (11.4,-11.1);

\draw[msg] (0,-1.0) -- node[lbl]{Nhập email/mật khẩu} (3.8,-1.0);
\draw[msg] (3.8,-2.0) -- node[lbl]{POST /api/auth/login} (7.6,-2.0);
\draw[msg] (7.6,-3.0) -- node[lbl]{Tìm user theo email} (11.4,-3.0);
\draw[return] (11.4,-4.0) -- node[lbl]{User + password hash} (7.6,-4.0);

\node[internal] (check) at (7.6,-5.0)
  {Kiểm tra tài khoản active\\và bcrypt.compare};
\node[internal] (issue) at (7.6,-6.2)
  {Tạo JWT};

\draw[return] (7.6,-7.2) -- node[lbl]{JWT + thông tin user an toàn} (3.8,-7.2);
\node[internal] (save) at (3.8,-8.2)
  {Lưu token vào localStorage};
\draw[msg] (3.8,-9.2) -- node[lbl]{Request sau kèm Bearer token} (7.6,-9.2);
\node[internal] (verify) at (7.6,-10.1)
  {jwt.verify; gán req.user};
\draw[return] (7.6,-11.0) -- node[lbl]{Dữ liệu hoặc 401/403} (3.8,-11.0);
\end{tikzpicture}
\caption{Trình tự xác thực và sử dụng JWT}
\label{fig:auth-sequence}
\end{figure}

Token không hợp lệ nhận mã 401 và phiên phía client bị xóa; mã 403 cho biết người dùng đã đăng nhập nhưng không đủ quyền. Luồng quên mật khẩu gồm yêu cầu OTP, xác minh và đặt mật khẩu mới; việc gửi email thật cần kiểm thử tích hợp riêng.
\clearpage
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        % Tầng 2: quyết định
        \node[decision] (decision) at (0,2.1) {Chọn đúng 1\\kết quả};

        % Tầng 3: các kết quả xử lý
        \node[flownode] (approve) at (-4.2,-0.4)
            {Duyệt và\\công khai};

        \node[flownode] (change) at (0,-0.4)
            {Yêu cầu sửa\\kèm lý do};

        \node[flownode] (reject) at (4.2,-0.4)
            {Từ chối\\kèm lý do};
        ...
        \node[
            ...
            {Cập nhật DB + notification + audit};

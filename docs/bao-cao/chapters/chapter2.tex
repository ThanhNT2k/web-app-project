...
\draw[arrow] (read) -- node[above,font=\footnotesize]{Tùy chọn} (login);
...
\draw[lifeline] (u.south) -- (0,-6.4);
\draw[lifeline] (fe.south) -- (3.8,-6.4);
\draw[lifeline] (api.south) -- (7.6,-6.4);
\draw[lifeline] (db.south) -- (11.4,-6.4);

\draw[msg] (0,-1.0) -- node[lbl]{Nhập email/mật khẩu} (3.8,-1.0);
\draw[msg] (3.8,-1.9) -- node[lbl]{POST /api/auth/login} (7.6,-1.9);
\draw[msg] (7.6,-2.8) -- node[lbl]{Tìm user} (11.4,-2.8);
\draw[return] (11.4,-3.6) -- node[lbl]{Bản ghi user} (7.6,-3.6);
\draw[return] (7.6,-4.4) -- node[lbl]{JWT + thông tin an toàn} (3.8,-4.4);
\draw[msg] (3.8,-5.2) -- node[lbl]{Request kèm Bearer token} (7.6,-5.2);
\draw[return] (7.6,-6.0) -- node[lbl]{Dữ liệu hoặc 401/403} (3.8,-6.0);
\end{tikzpicture}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        % Tầng 2: quyết định
        \node[decision] (decision) at (0,2.1) {Quyết định};

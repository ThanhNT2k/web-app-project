...
\begin{tikzpicture}[
  scale=0.86,
  transform shape,
  actor/.style={draw,rounded corners=3pt,minimum width=2.5cm,minimum height=0.8cm,align=center,font=\small\bfseries,fill=white},
...
\node[actor] (fe) at (3.7,0) {React + Axios};
\node[actor] (api) at (7.5,0) {Express API};
\node[actor] (db) at (11.3,0) {PostgreSQL};

\draw[lifeline] (u.south) -- (0,-13.3);
\draw[lifeline] (fe.south) -- (3.7,-13.3);
\draw[lifeline] (api.south) -- (7.5,-13.3);
\draw[lifeline] (db.south) -- (11.3,-13.3);

\node[phase] at (-0.3,-0.75) {A. Đăng nhập và cấp token};
\draw[msg] (0,-1.35) -- node[lbl]{Nhập email/mật khẩu} (3.7,-1.35);
\draw[msg] (3.7,-2.15) -- node[lbl]{POST /api/auth/login\\(không có Bearer token)} (7.5,-2.15);
\node[internal] at (7.5,-3.0) {Validate request};
\draw[msg] (7.5,-3.75) -- node[lbl]{Tìm user theo email} (11.3,-3.75);
\draw[return] (11.3,-4.55) -- node[lbl]{User + password hash} (7.5,-4.55);
\node[internal] at (7.5,-5.35)
  {Kiểm tra is\_active\\và bcrypt.compare};
\node[internal] at (7.5,-6.35)
  {jwt.sign(payload, secret, expiry)};
\draw[return] (7.5,-7.15) -- node[lbl]{200: JWT + user đã loại password} (3.7,-7.15);
\node[internal] at (3.7,-7.95)
  {Lưu cmc\_token và cmc\_user};

\node[phase] at (-0.3,-8.65) {B. Gọi API được bảo vệ};
\draw[msg] (3.7,-9.25) -- node[lbl]{Authorization: Bearer \{JWT\}} (7.5,-9.25);
\node[internal] at (7.5,-10.05)
  {authenticateToken: jwt.verify\\và gán payload vào req.user};
\node[internal] at (7.5,-11.05)
  {RBAC nếu route yêu cầu;\\sau đó controller xử lý};
\draw[msg] (7.5,-11.75) -- node[lbl]{Truy vấn/cập nhật theo req.user.id} (11.3,-11.75);
\draw[return] (11.3,-12.45) -- node[lbl]{Kết quả} (7.5,-12.45);
\draw[return] (7.5,-13.15) -- node[lbl]{2xx; hoặc 401 token lỗi;\\403 không đủ quyền} (3.7,-13.15);
\end{tikzpicture}

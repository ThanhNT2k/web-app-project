...
\node[actor,left=1.8cm of sys,yshift=2.0cm] (guest) {Guest};
\node[actor,left=1.8cm of sys] (user) {User};
\node[actor,left=1.8cm of sys,yshift=-2.0cm] (uploader) {Uploader};
\node[actor,right=1.8cm of sys,yshift=1.35cm] (moderator) {Moderator};
\node[actor,right=1.8cm of sys,yshift=-1.35cm] (admin) {Admin};
...
\begin{itemize}
  \item \textbf{Xác thực và tài khoản:} đăng ký, đăng nhập, đăng xuất, Google OAuth, quên/đặt lại mật khẩu bằng OTP và cập nhật hồ sơ;
  \item \textbf{Khám phá truyện:} danh sách, tìm kiếm, lọc theo tag hoặc danh mục, bảng xếp hạng và trang chi tiết;
  \item \textbf{Trải nghiệm đọc:} đọc chương theo URL thân thiện, lưu lịch sử, lưu chương đã đọc và tùy chọn hiển thị;
  \item \textbf{Tương tác:} theo dõi, đánh giá sao, bình luận, trả lời, bình chọn và báo cáo vi phạm;
  \item \textbf{Đăng nội dung:} tạo truyện, import tệp văn bản/EPUB, quản lý chương, tag và cộng tác viên;
  \item \textbf{Kiểm duyệt:} hàng chờ truyện, yêu cầu sửa, từ chối/duyệt, xử lý bình luận, hồ sơ và báo cáo;
  \item \textbf{Quản trị:} quản lý user, role, trạng thái tài khoản, truyện, từ khóa, báo cáo và audit log;
  \item \textbf{Tính năng hỗ trợ:} tóm tắt chương và gợi ý truyện bằng AI, notification, worker và hàng đợi tác vụ nền.
\end{itemize}
...
\begin{itemize}
  \item đặc tả yêu cầu và tài liệu kiến trúc trong thư mục \CodePath{docs};
  \item mã nguồn frontend, backend, schema và cấu hình triển khai;
  \item kết quả kiểm thử và build production tại thời điểm rà soát;
  \item ảnh chụp đúng phiên bản source do nhóm bổ sung.
\end{itemize}
...
Chương 1 đã trình bày bối cảnh, bài toán, mục tiêu, đối tượng sử dụng, phạm vi và yêu cầu phi chức năng của CMC Truyện. Trên cơ sở đó, Chương~\ref{chap:flows} phân tích chi tiết luồng hoạt động theo từng vai trò.
